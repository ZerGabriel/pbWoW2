...
$WH.$E = function(e)
{
	...
	else
	{
		...
		if($WH.Browser.ie6789 && e._button)
		{
			if(e._button & 4)
				e._button = 2; // Middle
			else if(e._button & 2)
...
$WH.rtrim = function(z,y){var a=z.length;while(--a>0&&z.charAt(a)==y);z=z.substring(0,a+1);if(z==y)z='';return z}
...
$WH.str_replace = function(z,a,b){while(z.indexOf(a)!=-1)z=z.replace(a,b);return z}
...
$WH.number_format = function(z){x=(''+parseFloat(z)).split('.');z=x[0];x=x.length>1?'.'+x[1]:'';if(z.length<=3)return z+x;return $WH.number_format(z.substr(0,z.length-3))+','+z.substr(z.length-3)+x}

$WH.is_array = function(a){return !!(a && a.constructor == Array)}
$WH.in_array = function(a,r,f,s){if(a==null)return -1;if(f)return $WH.in_arrayf(a,r,f,s);for(var i=s||0,len=a.length;i<len;++i){if(a[i]==r)return i}return -1}
$WH.in_arrayf = function(a,r,f,s){for(var i=s||0,len=a.length;i<len;++i){if(f(a[i])==r)return i}return -1}
$WH.rs = function(){var c=$WH.rs.random;var r='';for (var i=0;i<16;i++){var n=Math.floor(Math.random()*c.length);if(i==0&&n<11){n+=10};r+=c.substring(n,n+1);}return r;};
$WH.rs.random ="0123456789abcdefghiklmnopqrstuvwxyz";
$WH.isset = function(name)
{
...
// Get element
$WH.ge = function(z){if(typeof z != 'string') return z; return document.getElementById(z)}

// Get elements by tag name
$WH.gE = function(z,y){return z.getElementsByTagName(y)}

// Create element
$WH.ce = function(z, p, c){var r = document.createElement(z); if(p) $WH.cOr(r, p); if(c) $WH.ae(r, c); return r }

// Delete element
$WH.de = function(z){if(!z||!z.parentNode)return;z.parentNode.removeChild(z)}
...
// Empty element
$WH.ee = function(z,y){if(!y)y=0;while(z.childNodes[y])z.removeChild(z.childNodes[y])}
...
// Return false
$WH.rf = function(){return false}

// Return false only if no control key is pressed
$WH.rf2 = function(e){e=$WH.$E(e);if(e.ctrlKey||e.shiftKey||e.altKey||e.metaKey)return; return false}
...
// Stop propagation
$WH.sp = function(z){if(!z)z=event;if($WH.Browser.ie6789){z.cancelBubble=true}else{z.stopPropagation()}}
...
$WH.ns = function(n){if($WH.Browser.ie6789){n.onfocus=$WH.tb;n.onmousedown=n.onselectstart=n.ondragstart=$WH.rf;}}
...
// Copy object
$WH.cO = function(d, s) { for(var p in s) if(s[p] !== null && typeof s[p] == 'object' && s[p].length)d[p]=s[p].slice(0);else d[p]=s[p];return d; }
...
$WH.g_convertRatingToPercent = function(level, rating, value, classs)
{
	var ratingBases = $WH.g_convertRatingToPercent.RB,
		lookupH = $WH.g_convertRatingToPercent.LH;
		// We should be able to calculate H for 81-90. It approximates
		//     (82 / 52) * (131 / 63) * 3.9052 ^ ((level - 80) / 5)
		// but this formula wasn't quite close enough so I used hard-
		// coded values.     --Claire
	...
	if((rating == 12 || rating == 13 || rating == 14 || rating == 15) && level < 34)
		level = 34
...
$WH.g_convertScalingSpell = function(level, dist)
{
	var result = {},
		scalingValues = $WH.g_convertScalingSpell.SV,
		scalingDistributions = $WH.g_convertScalingSpell.SD,
		scaleDistribution,
		scaleValue;

	if(!scalingDistributions[dist])
	{
		if(g_user.roles & U_GROUP_ADMIN)
			alert('There are no spell scaling distributions for dist ' + dist);
		return result;
	}
    ...
    if(!scaleDistribution[3])
	{
		if(g_user.roles & U_GROUP_ADMIN)
			alert('This spell should not scale at all');
		return result;
	}
	else if(scaleDistribution[3] == -1)
	{
		if(g_user.roles & U_GROUP_ADMIN)
			alert('This spell should use the generic scaling distribution 12');
		scaleDistribution[3] = 12;
	}

	if(!scalingValues[level][scaleDistribution[3] - 1])
	{
		if(g_user.roles & U_GROUP_ADMIN)
			alert('Unknown category for spell scaling ' + scaleDistribution[3]);
		return result;
	}

	scaleValue = scalingValues[level][scaleDistribution[3] - 1];

	// Some spells scale less (or not at all) if used above a certain level
    if (scaleDistribution[13]) {
        scaleValue *= (Math.min(level, scaleDistribution[14]) + (scaleDistribution[13] * Math.max(0, level - scaleDistribution[14]))) / level;
    }

	result.cast    = Math.min(scaleDistribution[1], scaleDistribution[1] > 0 && level > 1 ? scaleDistribution[0] + (((level - 1) * (scaleDistribution[1] - scaleDistribution[0])) / (scaleDistribution[2] - 1)) : scaleDistribution[0]);
	result.effects = {};

	for(var i = 0; i < 3; ++i)
	{
		var x = scaleDistribution[ 4 + i],
			y = scaleDistribution[ 7 + i],
			z = scaleDistribution[10 + i],
			_ = result.effects[i + 1] = {};

		_.avg = x * scaleValue * (scaleDistribution[1] > 0 ? result.cast / scaleDistribution[1] : 1);
		_.min = Math.round(_.avg) - Math.floor(_.avg * y / 2);
		_.max = Math.round(_.avg) + Math.floor(_.avg * y / 2);
		_.pts = Math.round(z * scaleValue);
		_.avg = Math.max(Math.ceil(x), Math.round(_.avg));
	}

	result.cast = Math.round(result.cast / 10) / 100;
...
$WH.g_getDataSource = function()
{
	if($WH.isset('g_pageInfo'))
	{
		switch(g_pageInfo.type)
		{
			...
			case 200: if($WH.isset('g_petabilities')) return g_petabilities;
		}
	}
...
$WH.g_setJsonItemLevel = function(json, level)
{
	...
	var
		invSlot  = json.scaflags & 0xFF,
		subClass = (json.scaflags >> 8) & 0xFF,
		isArmor  = (json.scaflags & (1 << 16)) != 0,
		isWeapon = (json.scaflags & (1 << 17)) != 0,
		isCaster = (json.scaflags & (1 << 18)) != 0,
		scaleColumn;
	...
	switch(invSlot)
	{
		...
		case 7:  // Legs
		case 17: // 2H
			scaleColumn = 7; // statPrimary
			break;

		case 3:  // Shoulders
		case 12: // Trinket
			scaleColumn = 8; // statSecondary
			break;

		case 16: // Back
		case 11: // Finger
			scaleColumn = 9; // statTertiary
			break;

		case 15: // Ranged
			scaleColumn = 10; // statRanged
			break;

		case 23: // Held in offhand
		case 21: // Main hand
		case 22: // Offhand
		case 13: // 1H
			scaleColumn = 11; // stat1h
			break;
	...
	// Spell Power
	if(isCaster)
		json.splpwr = json.bonuses[45] = $WH.g_convertScalingFactor(level, 6 /* statSpellPower */);
	...
	if(isArmor)
	{
		...
		{
			case 3:  // Shoulders
				json.armor = $WH.g_convertScalingFactor(level, 11 + subClass /* armorShouldersCloth..armorShouldersPlate */);
				break;

			case 5:  // Chest
				json.armor = $WH.g_convertScalingFactor(level, 15 + subClass /* armorChestCloth..armorChestPlate */);
				break;

			case 1:  // Head
				json.armor = $WH.g_convertScalingFactor(level, 19 + subClass /* armorHeadCloth..armorHeadPlate */);
				break;

			case 7:  // Legs
				json.armor = $WH.g_convertScalingFactor(level, 23 + subClass /* armorLegsCloth..armorLegsPlate */);
				break;

			case 16: // Back
				json.armor = $WH.g_convertScalingFactor(level, 28 /* armorCloak */);
				break;
	...
	if(isWeapon)
	{
		...
		switch(invSlot)
		{
			...
			case 22: // Offhand
			case 13: // 1H
				json.dps = json[damageType + 'dps'] = $WH.g_convertScalingFactor(level, isCaster ? 2 /* dpsCaster1h */ : 0 /* dpsMelee1h */);
				damageRange = 0.3;
				break;

			case 17: // 2H
				json.dps = json[damageType + 'dps'] = $WH.g_convertScalingFactor(level, isCaster ? 3 /* dpsCaster2h */ : 1 /* dpsMelee2h */);
				damageRange = 0.2;
				break;

			case 15: // Ranged
				json.dps = json[damageType + 'dps'] = $WH.g_convertScalingFactor(level, subClass == 19 /* Wand */ ? 5 /* dpsWand */ : 4 /* dpsRanged */);
				damageRange = 0.3;
				break;

			default:
				json.dps = json[damageType + 'dps'] = 0;
				damageRange = 0;
		}

		json.dmgmin = json[damageType + 'dmgmin'] = Math.floor(json.dps * json.speed * (1.0 - damageRange));
		json.dmgmax = json[damageType + 'dmgmax'] = Math.floor(json.dps * json.speed * (1.0 + damageRange));
	}
	...
	if(json.gearscore != null)
	{
		...
		var equivLevel = Math.min(90, level + 1);
...
$WH.g_getJsonUpgrade = function(json, level, randId)
{
	var upgrade = {},
		levelAdjust = $WH.g_getJsonUpgrade.LA,
		armorValues = $WH.g_getJsonUpgrade.AV;

	for(var i in $WH.g_statToJson)
	{
		var statId = $WH.g_statToJson[i];
		if(json[statId])
		{
			if(statId != 'armor' && levelAdjust[level])
				upgrade[statId] = Math.floor(json[statId] * levelAdjust[level]) - json[statId];

			if(statId == 'armor' && armorValues[json.level] && armorValues[json.level + level])
			{
				if($WH.in_array([1,2,3,4], json.subclass) != -1) // Armor FIX bug if cloth no armor upgrade
					upgrade[statId] = Math.round(json[statId] * (armorValues[json.level + level][json.subclass - 1] / armorValues[json.level][json.subclass - 1])) - json[statId];

				if(json.subclass == 6) // Shield
					upgrade[statId] = armorValues[json.level + level][11 - json.name.charAt(0)] - json[statId];
			}
		}

        if(randId && json.subitems[randId].jsonequip && json.subitems[randId].jsonequip[statId])
            upgrade[statId] = Math.floor(json.subitems[randId].jsonequip[statId] * levelAdjust[level]) - json.subitems[randId].jsonequip[statId];
	}

    var dpsStats = ['mledmgmin', 'mledmgmax', 'mledps'];
    for (var i in dpsStats) {
        if (json[dpsStats[i]] && levelAdjust[level]) {
            upgrade[dpsStats[i]] = Math.floor(json[dpsStats[i]] * levelAdjust[level]) - json[dpsStats[i]];
        }
    }
...
$WH.g_getJsonReforge = function(json, reforgeId)
{
	if(!reforgeId)
	{
		...
		if(!json.__reforge.all)
		{
			json.__reforge.all = [];
			for(var reforgeId in $WH.g_reforgeStats)
			{
				var reforge = $WH.g_getJsonReforge(json, reforgeId)
				if(reforge.amount)
					json.__reforge.all.push(reforge);
			}
		}

		return json.__reforge.all;
	}

	if(!$WH.g_reforgeStats || !$WH.g_reforgeStats[reforgeId])
		return {};
	...
	if(!json.__statidx)
	{
		...
		for(var i in json)
		{
			if($WH.g_individualToGlobalStat[$WH.g_jsonToStat[i]])
				json.__statidx[$WH.g_individualToGlobalStat[$WH.g_jsonToStat[i]]] = json[i];
			else
				json.__statidx[$WH.g_jsonToStat[i]] = json[i];
		}
	}
	...
	if(!json.__reforge[reforgeId])
	{
		var reforge = json.__reforge[reforgeId] = $WH.dO($WH.g_reforgeStats[reforgeId]);
		json.__reforge[reforgeId].amount = Math.floor(reforge.v * (json.__statidx[reforge.i1] && !json.__statidx[reforge.i2] ? json.__statidx[reforge.i1] : 0));
	}

	return json.__reforge[reforgeId];
}

$WH.g_setTooltipLevel = function(tooltip, level, buff)
{
	...
	if(scaDist)
	{
		if(!tooltip.match(/<!--pts[0-9](:[0-9])?-->/g)) // Not a spell
		{
			...
			tooltip = tooltip.replace(/<span><!--stat(\d+)-->[-+]\d+(\D*?)<\/span>(<!--e-->)?(<!--ps-->)?(<br ?\/?>)?/gi, function(_all, statId, statText, e, ps, br) {
				var style, value = json.bonuses[statId];
				if(value)
				{
					value = (value > 0 ? '+' : '-') + value;
					style = '';
			...
			tooltip = tooltip.replace(/<span class="q2">(.*?)<!--rtg(\d+)-->\d+(.*?)<\/span>(<br \/>)?/gi, function(_all, prefix, ratingId, suffix, e, ps, br) {
				var style, value = json.bonuses[$WH.g_individualToGlobalStat[ratingId] || ratingId];
				if(value)
				{
					style = '';
					br = (br ? '<br />' : '');
		...
		else
		{
			...
			if(json.effects)
			{
				for(var i = 1; i < 4; ++i)
				{
					var effect = json.effects[i];

					tooltip = tooltip.replace(new RegExp('<!--pts' + i + '(:0)?-->(.+?)<', 'g'),		'<!--pts' + i + '$1-->' + (effect.min == effect.max ? effect.avg : effect.min + ' to ' + effect.max) + '<');
					tooltip = tooltip.replace(new RegExp('<!--pts' + i + ':1-->(.+?)<', 'g'),			'<!--pts' + i + ':1-->' + effect.min + '<');
					tooltip = tooltip.replace(new RegExp('<!--pts' + i + ':2-->(.+?)<', 'g'),			'<!--pts' + i + ':2-->' + effect.max + '<');
					tooltip = tooltip.replace(new RegExp('<!--pts' + i + ':3:(\\d+)-->(.+?)<', 'g'),	function(_all, ticks) { return '<!--pts' + i + ':3:' + ticks + '-->' + (effect.avg * ticks) + '<' });
					tooltip = tooltip.replace(new RegExp('<!--pts' + i + ':4-->(.+?)<', 'g'),			'<!--pts' + i + ':4-->' + effect.pts + '<');
				}
			}
		}
	}
	...
	// Level
	tooltip = tooltip.replace(/(<!--\?\d+:\d+:\d+:)\d+/, '$1' + level);
...
$WH.g_enhanceTooltip = function(tooltip, isStatic, useGets, showSlider, buff, knownSpells, chooseSpells, upgrade, numUpgrades, itemUpgradeDiv)
{
	var _ = typeof tooltip, spellJson;
    this._knownSpells = knownSpells;
	if(_ == 'number')
	{
		var arr = $WH.g_getDataSource(), id = tooltip;
		if(arr[id] && arr[id][(buff ? 'buff_' : 'tooltip_') + Locale.getName()])
		{
			tooltip = arr[id][(buff ? 'buff_' : 'tooltip_') + Locale.getName() + (upgrade ? ('_' + upgrade) : '')]
	...
	if(showSlider && Slider) // Make sure the class exists
	{
		...
		else
		{
			...
			if(_ && _[2] != _[3])
			{
				...
				this.slider.onmouseover = function(e) { $WH.Tooltip.showAtCursor(e, LANG.tooltip_changelevel2, 0, 0, 'q2'); };
    ...
    if (itemUpgradeDiv) {
        for (var i=1; i<=numUpgrades; ++i) {
            $(itemUpgradeDiv)
                .append('<input type="checkbox" id="item-upgrade-' + i + '" />')
                .append('<label for="item-upgrade-' + i + '"><a>' + $WH.sprintf(LANG['itemupgrade_format'], i) + '</a></label>')
                .append('<br />');

            $('#item-upgrade-' + i).change($WH.g_tooltipsUpgradeItem.bind(this, itemUpgradeDiv, i));
        }
        $(itemUpgradeDiv).toggle(!$(itemUpgradeDiv).is(':empty'));
    }
...
// upgrades the item tooltips
$WH.g_tooltipsUpgradeItem = function(itemUpgradeDiv, upgrade)
{
    var itemUpgrades = $('#' + itemUpgradeDiv.id + ' > input');
    var upgradeElement = itemUpgrades.get(upgrade-1);
    var checked = upgradeElement.checked;
    itemUpgrades.each(function(index, ele) {
        ele.checked = false;
    });
    upgradeElement.checked = checked;
    if (!checked) {
        upgrade = null;
    }
    var arr = $WH.g_getDataSource();
    var itemId = g_pageInfo['typeId'];
    if (arr[itemId] && arr[itemId]['tooltip_' + Locale.getName()]) {
        var sl = document.getElementById('sl' + itemId);
        this.innerHTML = '<table><tr><td>' + $WH.g_enhanceTooltip.call(this, itemId, true, true, sl, null, this._knownSpells, document.getElementById('ks' + itemId), upgrade, null, null) + '</td><th style="background-position: top right"></th></tr><tr><th style="background-position: bottom left"></th><th style="background-position: bottom right"></th></tr></table>';
        $WH.Tooltip.fixSafe(tt, 1, 1, sl);
    }
}
...
$WH.g_tooltipSpellsChange = function()
{
	...
	var chooseSpells = this.modified[0],
		spellJson    = this.modified[1],
		knownSpells  = [];

	$.each($('input:checked', chooseSpells), function(i, s) { knownSpells.push(parseInt(s.id.replace('known-', ''))) });
...
{
	var raceXclass = {
		22: {6:1,11:1,3:1,8:1,5:1,4:1,9:1,1:1},          // worgen
		 9: {6:1,3:1,8:1,5:1,4:1,7:1,9:1,1:1},           // goblin
		10: {6:1,3:1,8:1,10:1,2:1,5:1,4:1,9:1},          // bloodelf
		11: {6:1,3:1,8:1,10:1,2:1,5:1,7:1,1:1},          // draenei
		 3: {6:1,3:1,8:1,10:1,2:1,5:1,4:1,7:1,9:1,1:1},  // dwarf
		 7: {6:1,8:1,10:1,5:1,4:1,9:1,1:1},              // gnome
		 1: {6:1,3:1,8:1,10:1,2:1,5:1,4:1,9:1,1:1},      // human
		 4: {6:1,11:1,3:1,8:1,10:1,5:1,4:1,1:1},         // nightelf
		 2: {6:1,3:1,8:1,10:1,4:1,7:1,9:1,1:1},          // orc
		 6: {6:1,11:1,3:1,10:1,2:1,5:1,7:1,1:1},         // tauren
		 8: {6:1,11:1,3:1,8:1,10:1,5:1,4:1,7:1,9:1,1:1}, // troll
		 5: {6:1,3:1,8:1,10:1,5:1,4:1,9:1,1:1},          // scourge
		24: {3:1,8:1,10:1,5:1,4:1,7:1,1:1},              // pandaren (neutral)
		25: {3:1,8:1,10:1,5:1,4:1,7:1,1:1},              // pandaren (alliance)
		26: {3:1,8:1,10:1,5:1,4:1,7:1,1:1}               // pandaren (horde)
	};
	...
	if(raceId == 22 || raceId == 9)
		return 'race_' + g_file_races[raceId] + '_' + g_file_genders[gender];
